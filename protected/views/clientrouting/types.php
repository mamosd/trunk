...
<div class="titleWrap">
    ...
    <ul>
        ...
        <li class="seperator"><img src="img/icons/add.png" alt="add" />
            <a href="<?php echo $this->createUrl('clientrouting/type');?>">Add New</a>
        </li>
    </ul>
</div>
...
<tbody>
    ...
    ?>
        <tr class="row<?php echo (($i%2)+1) ?>">
          ...
          <td>
              <img src="<?php echo $baseUrl ?>/img/icons/<?php echo $types[$i]->IsLive == 1 ? 'accept' : 'cancel' ?>.png" />
              
          </td>
          <td>
              <a href="<?php echo $this->createUrl('clientrouting/type', array('id'=>$types[$i]->titleId));?>">
    ...
    ?>
        <tr class="row1">
            <td colspan="4">
                <div class="infoBox">There are no types setup on the system, <a href="<?php echo $this->createUrl('clientrouting/title');?>">add a new one now</a>.</div>
            </td>
        </tr>
    <?php
    endif;
...
$(function(){
    $("#ddlFilter").change(function(){
        var url = "<?php echo $this->createUrl('clientrouting/types', array_merge( $_GET,array('f' => '999') ) ) ?>";
        var filter = $(this).val();
    ...
    $("#printCentres").change(function(){
        var url = "<?php echo $this->createUrl('clientrouting/types',  array_merge( $_GET,array('fp' => '999') ) ) ?>";
        var filter = $(this).val();
        url = url.replace('999', filter);
        location.href = url;
    });
