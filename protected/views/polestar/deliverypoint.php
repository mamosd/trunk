<?php
    $this->breadcrumbs=array(
                ...
                array('label'=>'Delivery Points', 'url'=>array('polestar/deliverypoints')),
...
<?php $form=$this->beginWidget('CActiveForm', array(
    ...
    <div>
        ...
        <?php echo $form->textField($model,'company', array('size'=>'45')); ?>
    </div>
    ...
    <div>
        ...
        <?php echo $form->textField($model,'area', array('size'=>'45')); ?>
    </div>
...
?>
    $("#openingShowHide<?=$day?>").click(function(){
        ...
        {
            $("#openingDiv<?=$day?>1").html("<input type='hidden' name='PolestarDeliveryPointForm[OpeningStart<?=$day?>]' value='' />\n\
                                <input type='hidden' name='PolestarDeliveryPointForm[OpeningEnd<?=$day?>]' value='' />");
            $("#openingDiv<?=$day?>").hide();
        }
        else
        {
...
?>
    $("#closingShowHide<?=$day?>").click(function(){
        ...
        {
            $("#closingDiv<?=$day?>1").html("<input type='hidden' name='PolestarDeliveryPointForm[ClosingStart<?=$day?>]' value='' />\n\
                                <input type='hidden' name='PolestarDeliveryPointForm[ClosingEnd<?=$day?>]' value='' />");
            $("#closingDiv<?=$day?>").hide();
        }
        else
        {
...
?>
    ...
    <div class="titleWrap">
        <?php 
            if (Login::checkPermission(Permission::PERM__FUN__POLESTAR__DEL_POINT_EDIT)) {
    ...
    <div class="titleWrap">
        ...
        <li class="seperator">
            ...
            <img height="16" width="16" alt="add" src="img/icons/add.png">
                <a href="<?php echo $this->createUrl('polestar/deliverypointContact', array('ui'=>'popUp','DeliveryPointId'=>$_GET['id']));?>" title="Add new Contact" class="colorBoxIFrame">Add Contact</a>
            <?php endif; ?>
        </li>
    </ul>
    </div>
...
<tbody>
    ...
    ?>
        <tr class="row<?php echo (($i%2)+1) ?>">
          ...
          <td style="white-space: nowrap;">
            <?php if (Login::checkPermission(Permission::PERM__FUN__POLESTAR__DEL_POINT_EDIT)) : ?>
              <a class="lnkDelete"
                  href="<?php echo $this->createUrl('polestar/deliveryPointcontactdelete', array('contactid'=>$contacts[$i]->ContactId,'deliverypointid'=>$_GET['id']));?>">
    ...
    ?>
        <tr class="row1">
            <td colspan="20">
                <div class="infoBox">There are no contacts for this delivery point.</div>
            </td>
        </tr>
    <?php
    endif;
...
{
    echo '<div class="infoBox">Delivery Point should be added before assigning contacts to it.</div>';
}
?>
